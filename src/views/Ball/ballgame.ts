...
export default async function BallGame() {
  Layout();
  const layoutContent = document.getElementById("content") as HTMLDivElement;

  const ballgame = `
        <canvas id="canvas" tabindex="0" width="840" height="640">
        Animations are not supported in older browsers.
        </canvas>
    `;

  layoutContent.innerHTML = ballgame;
